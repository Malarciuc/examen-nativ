...
window.onload = function(){
    ...
    document.querySelector('.cmd').addEventListener('click', function () {
        let name = document.querySelector('#myModal .name').textContent;
...
class Configurator{
    createElement(){
      ...
      for (let i = 0; i < json.produts.length; i++) {
          ...
          <div id="card-${i}" class="card pizza ">
              <span class="card-body">
              <img src="${json.produts[i].src}"></img><br><span class="productName"> ${json.produts[i].name}</span><br><span class="productPrice">${json.produts[i].price}</span><br>${json.produts[i].description}
              <br>
              <button type="button" class="btn1 toCart btn-info" onclick="cart.add({'name': '${json.produts[i].name}', 'price' : '${json.produts[i].price}' })">В корзину</button>
